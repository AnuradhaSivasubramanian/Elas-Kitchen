...
class Questionaire extends Component {
  ...
  handlerdata = () => {
    ...
    axios
      .get(
        `https://api.spoonacular.com/recipes/complexSearch?apiKey=${MY_KEY}&includeIngredients=${ingredients}number=1`
      )
      .then(res => {
        this.setState({ id: res.data.results[0].id});
        this.setState({ recipeMount: !this.state.recipeMount });
        this.setState({recipeImage:res.data.results[0].image})
  ...
  getAnalizedInst=(id)=>{
    axios.get(`https://api.spoonacular.com/recipes/${id}/analyzedInstructions?apiKey=${MY_KEY}`)
...
render() {
  return (
    <div>
        ...
        <Recipe recipes={this.state.recipes} recipeImage={this.state.recipeImage}  />:
